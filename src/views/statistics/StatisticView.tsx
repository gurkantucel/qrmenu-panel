...
import { useGetStatisticAppointmentQuery, useGetStatisticPatientQuery, useGetStatisticReadQuery } from 'reduxt/features/statistic/statistic-api'
...
const StatisticView = () => {
    ...
    const { data: getStatisticPatientData, isLoading: isStatisticPatientLoading, isFetching: isStatisticPatientFetching } = useGetStatisticPatientQuery({ daily: false, start_date: dayjs().startOf('year').format('YYYY-MM-DD HH:mm:ss'), end_date: dayjs().endOf('year').format('YYYY-MM-DD HH:mm:ss') })
        ...
        <>
            ...
            <Grid container spacing={3}>
                {isStatisticLoading || isStatisticFetching ? <Grid item xs={12}>
                    ...
                    <Grid item xs={12} sm={6} md={4} lg={3}>
                        <AnalyticEcommerce title={intl.formatMessage({ id: "numberOfOffers" })} count={getStatisticData.data.quoteStats.total.toString() ?? "0"} percentage={getStatisticData.data.quoteStats.increase} extra={getStatisticData.data?.quoteStats?.this_month?.toString() ?? "0"} />
                    </Grid>

                    {getStatisticData.data.mostAppointmentProcess != null && <Grid item xs={12} lg={4} sm={6}>
                        <RoundIconCard
                            primary={intl.formatMessage({ id: "mostTransactions" })}
                    ...
                    {getStatisticData.data.mostPerson != null && <Grid item xs={12} lg={4} sm={6}>
                        <RoundIconCard
                            primary={intl.formatMessage({ id: "mostTransactionsMadeEmployee" })}
                    ...
                    {getStatisticData.data.mostProcessDay != null && <Grid item xs={12} lg={4} md={12}>
                        <RoundIconCard
                            primary={intl.formatMessage({ id: "mostTransactionsMadeDay" })}
                ...
                {isStatisticAppointmentLoading || isStatisticAppointmentFetching ? <Grid item xs={12}>
                    ...
                    <Grid item xs={12}>
                        <RepeatCustomerRate title={intl.formatMessage({ id: "numberOfAppointments" })} data={getStatisticAppointmentData.data} />
                    </Grid>
                </Grid> : <></>}
                {isStatisticPatientLoading || isStatisticPatientFetching ? <Grid item xs={12}>
                    <MainCard><CustomScaleLoader /></MainCard>
                </Grid> : getStatisticPatientData?.data != null ? <Grid item xs={12} spacing={3} sx={{ marginTop: 1 }}>
                    <Grid item xs={12}>
                        <RepeatCustomerRate title={intl.formatMessage({ id: "numberOfPatients" })} data={getStatisticPatientData.data} />
                    </Grid>
                </Grid> : <></>}
            </Grid>
        </>
