...
const UpdateMakeAnOfferModal = (props: Props) => {
    ...
    const [updateMakeAnOffer, { isLoading: updateMakeAnOfferIsLoading, data: updateMakeAnOfferResponse, error: updateMakeAnOfferError }] = useUpdateMakeAnOfferMutation();


    useEffect(() => {
        if (open == true && modalType == ModalEnum.updateMakeAnOffer) {
    ...
    useEffect(() => {
        if (open == true && modalType == ModalEnum.updateMakeAnOffer && getReadMakeAnOfferData?.data != null) {
    ...
    useEffect(() => {
        if (updateMakeAnOfferResponse) {
            enqueueSnackbar(updateMakeAnOfferResponse.message, {
                variant: updateMakeAnOfferResponse?.status == true ? 'success' : 'error', anchorOrigin: {
                    vertical: 'bottom',
                    horizontal: 'right'
                }
            },)
            if (updateMakeAnOfferResponse?.status == true) {
                handleClose();
        ...
        if (updateMakeAnOfferError) {
            const error = updateMakeAnOfferError as any;
    ...
    }, [updateMakeAnOfferResponse, updateMakeAnOfferError])
    ...
    return (
        <>
            <Dialog open={open && modalType == ModalEnum.updateMakeAnOffer} onClose={handleClose} fullScreen>
                                ...
                                <DialogActions sx={{ marginTop: 5 }}>
                                    ...
                                    <AnimateButton>
                                        <Button disableElevation
                                            disabled={isSubmitting || updateMakeAnOfferIsLoading || (values.patient_id == null)} type="submit" variant="contained" color="primary">
                                            {(updateMakeAnOfferIsLoading) && <PuffLoader size={20} color='white' />}
                                            {(updateMakeAnOfferIsLoading == false) && intl.formatMessage({ id: "save" })}
                                        </Button>
                                    </AnimateButton>
                                </DialogActions>
                            </Box>
...
export default UpdateMakeAnOfferModal