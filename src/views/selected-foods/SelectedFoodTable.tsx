...
import { useGetSelectedFoodListQuery } from 'reduxt/features/selected-food/selected-food-api';
import { SelectedFoodData } from 'reduxt/features/selected-food/models/selected-food-model';
import AddSelectedFoodModal from './AddSelectedFoodModal';
import DeleteSelectedFoodModal from './DeleteSelectedFoodModal';
import UpdateStatusSelectedFoodModal from './UpdateStatusSelectedFoodModal';
import UpdateSelectedFoodOrderModal from './UpdateSelectedFoodOrderModal';

const columnHelper = createColumnHelper<SelectedFoodData>()
...
const SelectedFoodTable = () => {
    ...
    const breadcrumbLinks = [
        ...
        { title: `${intl.formatMessage({ id: "chefsPicks" })}` },
    ];

    const columns = useMemo<ColumnDef<SelectedFoodData, any>[]>(() => [
                    ...
                    <Tooltip title={intl.formatMessage({ id: info.row.original.status == true ? "makePassive" : "makeActive" })}>
                        <IconButton
                            ...
                            onClick={(e: any) => {
                                e.stopPropagation();
                                dispatch(setModal({
                                    open: true, modalType: ModalEnum.updateStatusSelectedFood,
                                    id: info.row.original.id,
                                    title: info.row.original.title.tr,
                                    data: { title: info.row.original.title, selectedFoodId: info.row.original.id, status: info.row.original.status }
                                }))
                            }}
                        >
                    ...
                    <Tooltip title={intl.formatMessage({ id: "delete" })}>
                        <IconButton
                            ...
                            onClick={(e: any) => {
                                e.stopPropagation();
                                dispatch(setModal({
                                    open: true, modalType: ModalEnum.deleteSelectedFood,
                                    id: info.row.original.id,
                                    title: info.row.original.title.tr,
                                    data: { title: info.row.original.title, selectedFoodId: info.row.original.id }
                                }))
                            }}
                        >
                            <Trash />
    ...
    return (
        <>
            <Breadcrumbs custom heading={`${intl.formatMessage({ id: "chefsPicks" })}`} links={breadcrumbLinks} />
            <MainCard content={false}>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={{ xs: 2, sm: 0 }} justifyContent={"space-between"} alignItems={{ xs: "normal", sm: "center" }} sx={{ padding: 2 }}>
                    ...
                    <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                        {(getMenuListData?.data?.length ?? 0) > 0  && <UpdateSelectedFoodOrderModal branchSlug={branchSlug} />}
                        <AddSelectedFoodModal branch={selectedBranch} />
                        <UpdateStatusSelectedFoodModal />
                        <DeleteSelectedFoodModal />
                    </Stack>
                </Stack>
                <ScrollX>
