...
import { Add, CloseSquare, DocumentUpload } from "iconsax-react"
...
const AddFoodModal = () => {
    ...
    useEffect(() => {
        if (createFoodResponse) {
            enqueueSnackbar(createFoodResponse.message, {
                variant: createFoodResponse?.success == true ? 'success' : 'error', anchorOrigin: {
                    vertical: 'bottom',
                    horizontal: 'right'
                }
            },)
                                                    ...
                                                    {values.branches.map((branch, branchIndex) => {
                                                        ...
                                                        return (<Grid container spacing={3} key={branchIndex} sx={{ borderBottom: "1px solid #dedede", paddingBottom: 3 }}>
                                                            <Grid item xs={12} md={1}>
                                                                <InputLabel htmlFor="title_en" sx={{ fontWeight: 500, marginBottom: 2 }}>{"Ekle"}</InputLabel>
